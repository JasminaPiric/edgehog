...
> = {
  ...
  DEPLOYING: { message: stateMessages.DEPLOYING, color: "bg-light text-muted" },
...
const DeployedApplicationsTable = ({
  ...
  const groupedDeployments = useMemo(() => {
    ...
    for (const edge of edges) {
      ...
      const dep = mapDeploymentNode(edge.node);

      if (!map.has(dep.applicationId)) {
        map.set(dep.applicationId, {
          applicationId: dep.applicationId,
          applicationName: dep.applicationName,
          releases: [],
        });
      }
      map.get(dep.applicationId)!.releases.push(dep);
    }

    return Array.from(map.values()).map((app) => ({
      ...app,
      releases: app.releases.sort((a, b) =>
  ...
  if (!groupedDeployments.length) {
    return (
      <FormattedMessage id="noApps" defaultMessage="No deployed applications" />
    );
  }
